...
import addPreceptor from "../actions/add-preceptor";
...
export default function AddPreceptorModal() {
  const [open, setOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [rotations, setRotations] = useState<Array<{ id: string; name: string; location: string }>>([]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!name.trim() || !specialty.trim() || !rotationName.trim() || !rotationLocation.trim()) {
    ...
    try {
      const preceptorId = await addPreceptor({
        name: name.trim(),
      ...
      if (preceptorId) {
        setOpen(false);
        router.push(`/preceptor/${preceptorId}`);
        router.refresh();
      } else {
        setError("Failed to add preceptor. Please try again.");
        setIsSubmitting(false);
      }
    } catch (err) {
  ...
  return (
    <Dialog open={open} onOpenChange={(newOpen) => {
      ...
      <DialogTrigger asChild>
        <Button>+ Add Preceptor</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Add New Preceptor</DialogTitle>
          <DialogDescription>
            Add a new preceptor to the database
          </DialogDescription>
        </DialogHeader>
